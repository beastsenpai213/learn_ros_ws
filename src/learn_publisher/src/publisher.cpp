...
public:
    ...
    publisher_node() : Node("my_publisher_node")
    {
        //配置定时器
        my_timer = this->create_wall_timer(
            std::chrono::milliseconds(100),/*配置该定时器每100ms执行一次*/
            std::bind(&publisher_node::timer_callback, this)/*配置该定时器的回调函数timer_callback*/
            );
...
int main(int argc, char** argv) {
  ...
  auto node = std::make_shared<publisher_node>();//创建节点指针
